...
async function showPage(name) {
    ...
    const machinesDropdown = document.getElementById('machinesDropdown');
    if (machinesDropdown) {
        const isMachinesPage = name === 'vending' || name === 'incinerator';
        machinesDropdown.querySelector('.nav-dropdown-toggle').classList.toggle('active', isMachinesPage);
    }
    ...
    if (name === 'vending') {
        initCardsSlider('vendingCardsSlider', 'vendingCardsTrack', 'vendingCardsDots');
    }
    return false;
}
...
document.addEventListener('DOMContentLoaded', function() {
    updateAuthUI();
    initHeroSlider();
    if (document.getElementById('page-vending') && document.getElementById('page-vending').classList.contains('active')) {
        initCardsSlider('vendingCardsSlider', 'vendingCardsTrack', 'vendingCardsDots');
    }
});
...
function initHeroSlider() {
    ...
    /* Build dot indicators dynamically to match the number of slides */
    dotsWrap.innerHTML = '';
    slides.forEach((_, i) => {
...
/* Generic slider used for the Vending Machine page's use-case cards.
   Works via native horizontal scroll + snap, with arrow buttons and
   dot indicators that both drive and reflect the current scroll position. */
const cardsSliderState = {};

function initCardsSlider(sliderId, trackId, dotsId) {
    const track = document.getElementById(trackId);
    const dotsWrap = document.getElementById(dotsId);
    if (!track || !dotsWrap) return;

    const cards = track.querySelectorAll('.vending-card');
    if (!cards.length) return;

    /* Avoid re-binding the scroll listener every time the page is revisited */
    if (cardsSliderState[sliderId]) {
        updateCardsDots(trackId, dotsId);
        return;
    }
    cardsSliderState[sliderId] = true;
    ...
    cards.forEach((_, i) => {
        ...
        dot.onclick = () => scrollToCard(trackId, i);
        dotsWrap.appendChild(dot);
    });
    ...
    track.addEventListener('scroll', () => {
        ...
        scrollTimeout = setTimeout(() => updateCardsDots(trackId, dotsId), 100);
    });
}

function getCardsPerView(track) {
    const cardWidth = track.querySelector('.vending-card').getBoundingClientRect().width;
    const gap = 24;
    return Math.max(1, Math.round(track.clientWidth / (cardWidth + gap)));
}

function slideCards(sliderId, direction) {
    const track = document.getElementById(sliderId === 'vendingCardsSlider' ? 'vendingCardsTrack' : sliderId);
    if (!track) return;
    const card = track.querySelector('.vending-card');
    if (!card) return;
    const cardWidth = card.getBoundingClientRect().width + 24; /* + gap */
    track.scrollBy({ left: direction * cardWidth, behavior: 'smooth' });
}

function scrollToCard(trackId, index) {
    const track = document.getElementById(trackId);
    if (!track) return;
    const card = track.querySelectorAll('.vending-card')[index];
    if (!card) return;
    track.scrollTo({ left: card.offsetLeft - track.offsetLeft, behavior: 'smooth' });
}

function updateCardsDots(trackId, dotsId) {
    const track = document.getElementById(trackId);
    const dotsWrap = document.getElementById(dotsId);
    if (!track || !dotsWrap) return;
    const cards = Array.from(track.querySelectorAll('.vending-card'));
    const dots = Array.from(dotsWrap.querySelectorAll('.cards-dot'));
    if (!cards.length || !dots.length) return;

    /* Find whichever card's left edge is closest to the track's current scroll position */
...
const TRANSLATIONS = {
  en: {
    "nav.home":"Home","nav.about":"About Us","nav.products":"Products","nav.machines":"Sanitary Pad Machines","nav.vendingSub":"Sanitary Pad Vending Machine","nav.incineratorSub":"Sanitary Pad Incinerator Machine",
    ...
    "incinerator.heroTitle":"🔥 Sanitary Pad Incinerator Machine",
    "incinerator.heroSub":"Safe, hygienic, and eco-friendly disposal of sanitary waste — protecting health and the environment together.",
    "incinerator.mainTitle":"Responsible Sanitary Waste Disposal",
    "incinerator.p1":"Our sanitary pad incinerator machines provide a safe and hygienic way to dispose of used sanitary napkins on-site, eliminating the health and environmental risks of manual handling or landfill disposal.",
    "incinerator.p2":"Compact and easy to operate, these machines are designed for restrooms in schools, hospitals, offices, and public facilities — reducing odor, contamination, and the spread of infection.",
    "incinerator.p3":"Available on both <strong>purchase</strong> and <strong>rental/lease</strong> basis, with installation and maintenance support from our team.",
    "incinerator.f1":"Odorless, smokeless combustion technology","incinerator.f2":"Compact design for restroom installation",
    "incinerator.f3":"Reduces health hazards from manual disposal","incinerator.f4":"Eco-friendly, reduces landfill waste",
    "incinerator.f5":"Ideal for schools, hospitals &amp; offices","incinerator.f6":"Available on rental or purchase basis",
    "incinerator.cta":"Enquire Now",
    "incinerator.c1t":"Schools &amp; Colleges","incinerator.c1p":"Safe, hygienic disposal for girls' washrooms, removing the need for manual waste handling by staff.",
    "incinerator.c2t":"Hospitals &amp; Clinics","incinerator.c2p":"Supports infection control protocols by safely incinerating sanitary and biomedical waste on-site.",
    "incinerator.c3t":"Corporate Offices","incinerator.c3p":"A hygienic, responsible facility upgrade that supports female employees and sustainability goals.",
    "incinerator.c4t":"Factories &amp; Industries","incinerator.c4p":"Robust units suited for industrial restroom facilities with high footfall.",
    "incinerator.c5t":"Public Spaces","incinerator.c5p":"Malls, bus stations, and railway stations — enabling safe disposal wherever vending machines are installed.",
    "incinerator.c6t":"Eco-Friendly Disposal","incinerator.c6p":"Reduces landfill burden and environmental contamination from improperly discarded sanitary waste.",
  ...
  es: {
    "nav.home":"Inicio","nav.about":"Nosotros","nav.products":"Productos","nav.machines":"Máquinas de Toallas Sanitarias","nav.vendingSub":"Máquina Expendedora de Toallas Sanitarias","nav.incineratorSub":"Incinerador de Toallas Sanitarias",
    ...
    "incinerator.heroTitle":"🔥 Incinerador de Toallas Sanitarias",
    "incinerator.heroSub":"Eliminación segura, higiénica y ecológica de residuos sanitarios — protegiendo la salud y el medio ambiente juntos.",
    "incinerator.mainTitle":"Eliminación Responsable de Residuos Sanitarios",
    "incinerator.p1":"Nuestros incineradores de toallas sanitarias ofrecen una forma segura e higiénica de eliminar toallas sanitarias usadas en el sitio, eliminando los riesgos sanitarios y ambientales del manejo manual o la disposición en vertederos.",
    "incinerator.p2":"Compactas y fáciles de operar, estas máquinas están diseñadas para baños en escuelas, hospitales, oficinas e instalaciones públicas — reduciendo el olor, la contaminación y la propagación de infecciones.",
    "incinerator.p3":"Disponible tanto en <strong>compra</strong> como en <strong>alquiler/arrendamiento</strong>, con soporte de instalación y mantenimiento de nuestro equipo.",
    "incinerator.f1":"Tecnología de combustión sin olor y sin humo","incinerator.f2":"Diseño compacto para instalación en baños",
    "incinerator.f3":"Reduce riesgos de salud por manejo manual","incinerator.f4":"Ecológico, reduce residuos en vertederos",
    "incinerator.f5":"Ideal para escuelas, hospitales y oficinas","incinerator.f6":"Disponible en alquiler o compra",
    "incinerator.cta":"Consultar Ahora",
    "incinerator.c1t":"Escuelas y Universidades","incinerator.c1p":"Eliminación segura e higiénica para baños de niñas, eliminando la necesidad de manejo manual de residuos por parte del personal.",
    "incinerator.c2t":"Hospitales y Clínicas","incinerator.c2p":"Apoya los protocolos de control de infecciones incinerando de forma segura los residuos sanitarios y biomédicos en el sitio.",
    "incinerator.c3t":"Oficinas Corporativas","incinerator.c3p":"Una mejora higiénica y responsable de las instalaciones que apoya a las empleadas y los objetivos de sostenibilidad.",
    "incinerator.c4t":"Fábricas e Industrias","incinerator.c4p":"Unidades robustas adecuadas para instalaciones de baños industriales con alto tráfico.",
    "incinerator.c5t":"Espacios Públicos","incinerator.c5p":"Centros comerciales, estaciones de autobús y estaciones de tren — permitiendo una eliminación segura donde se instalen máquinas expendedoras.",
    "incinerator.c6t":"Eliminación Ecológica","incinerator.c6p":"Reduce la carga en vertederos y la contaminación ambiental por residuos sanitarios desechados incorrectamente.",
  ...
  de: {
    "nav.home":"Startseite","nav.about":"Über Uns","nav.products":"Produkte","nav.machines":"Damenbinden-Maschinen","nav.vendingSub":"Damenbinden-Verkaufsautomat","nav.incineratorSub":"Damenbinden-Verbrennungsanlage",
    ...
    "incinerator.heroTitle":"🔥 Damenbinden-Verbrennungsanlage",
    "incinerator.heroSub":"Sichere, hygienische und umweltfreundliche Entsorgung von Hygieneabfällen — Gesundheit und Umwelt gemeinsam schützen.",
    "incinerator.mainTitle":"Verantwortungsvolle Entsorgung von Hygieneabfällen",
    "incinerator.p1":"Unsere Damenbinden-Verbrennungsanlagen bieten eine sichere und hygienische Möglichkeit, gebrauchte Damenbinden vor Ort zu entsorgen, wodurch die Gesundheits- und Umweltrisiken der manuellen Handhabung oder Deponierung entfallen.",
    "incinerator.p2":"Kompakt und einfach zu bedienen, sind diese Maschinen für Toiletten in Schulen, Krankenhäusern, Büros und öffentlichen Einrichtungen konzipiert — sie reduzieren Geruch, Kontamination und die Ausbreitung von Infektionen.",
    "incinerator.p3":"Erhältlich sowohl zum <strong>Kauf</strong> als auch zur <strong>Miete/Leasing</strong>, mit Installations- und Wartungsunterstützung durch unser Team.",
    "incinerator.f1":"Geruchlose, rauchfreie Verbrennungstechnologie","incinerator.f2":"Kompaktes Design für die Installation in Toiletten",
    "incinerator.f3":"Reduziert Gesundheitsrisiken durch manuelle Entsorgung","incinerator.f4":"Umweltfreundlich, reduziert Deponieabfälle",
    "incinerator.f5":"Ideal für Schulen, Krankenhäuser und Büros","incinerator.f6":"Erhältlich zur Miete oder zum Kauf",
    "incinerator.cta":"Jetzt Anfragen",
    "incinerator.c1t":"Schulen und Hochschulen","incinerator.c1p":"Sichere, hygienische Entsorgung für Mädchentoiletten, wodurch die manuelle Abfallhandhabung durch das Personal entfällt.",
    "incinerator.c2t":"Krankenhäuser und Kliniken","incinerator.c2p":"Unterstützt Infektionskontrollprotokolle durch sichere Verbrennung von Hygiene- und biomedizinischen Abfällen vor Ort.",
    "incinerator.c3t":"Firmenbüros","incinerator.c3p":"Eine hygienische, verantwortungsvolle Aufwertung der Einrichtung, die weibliche Mitarbeiter und Nachhaltigkeitsziele unterstützt.",
    "incinerator.c4t":"Fabriken und Industrie","incinerator.c4p":"Robuste Einheiten, geeignet für industrielle Toilettenanlagen mit hohem Besucheraufkommen.",
    "incinerator.c5t":"Öffentliche Räume","incinerator.c5p":"Einkaufszentren, Busbahnhöfe und Bahnhöfe — ermöglichen eine sichere Entsorgung überall dort, wo Verkaufsautomaten installiert sind.",
    "incinerator.c6t":"Umweltfreundliche Entsorgung","incinerator.c6p":"Reduziert die Deponiebelastung und Umweltverschmutzung durch unsachgemäß entsorgte Hygieneabfälle.",
  ...
  fr: {
    "nav.home":"Accueil","nav.about":"À Propos","nav.products":"Produits","nav.machines":"Machines pour Serviettes Hygiéniques","nav.vendingSub":"Distributeur de Serviettes Hygiéniques","nav.incineratorSub":"Incinérateur de Serviettes Hygiéniques",
    ...
    "incinerator.heroTitle":"🔥 Incinérateur de Serviettes Hygiéniques",
    "incinerator.heroSub":"Élimination sûre, hygiénique et écologique des déchets sanitaires — protégeant la santé et l'environnement ensemble.",
    "incinerator.mainTitle":"Élimination Responsable des Déchets Sanitaires",
    "incinerator.p1":"Nos incinérateurs de serviettes hygiéniques offrent un moyen sûr et hygiénique d'éliminer les serviettes hygiéniques usagées sur place, éliminant les risques sanitaires et environnementaux de la manipulation manuelle ou de la mise en décharge.",
    "incinerator.p2":"Compactes et faciles à utiliser, ces machines sont conçues pour les toilettes des écoles, hôpitaux, bureaux et installations publiques — réduisant les odeurs, la contamination et la propagation des infections.",
    "incinerator.p3":"Disponible à l'<strong>achat</strong> et à la <strong>location/crédit-bail</strong>, avec support d'installation et de maintenance de notre équipe.",
    "incinerator.f1":"Technologie de combustion sans odeur et sans fumée","incinerator.f2":"Conception compacte pour l'installation dans les toilettes",
    "incinerator.f3":"Réduit les risques sanitaires liés à l'élimination manuelle","incinerator.f4":"Écologique, réduit les déchets en décharge",
    "incinerator.f5":"Idéal pour les écoles, hôpitaux et bureaux","incinerator.f6":"Disponible en location ou à l'achat",
    "incinerator.cta":"Demander Maintenant",
    "incinerator.c1t":"Écoles et Collèges","incinerator.c1p":"Élimination sûre et hygiénique pour les toilettes des filles, supprimant le besoin de manipulation manuelle des déchets par le personnel.",
    "incinerator.c2t":"Hôpitaux et Cliniques","incinerator.c2p":"Soutient les protocoles de contrôle des infections en incinérant en toute sécurité les déchets sanitaires et biomédicaux sur place.",
    "incinerator.c3t":"Bureaux d'Entreprise","incinerator.c3p":"Une amélioration hygiénique et responsable des installations qui soutient les employées et les objectifs de durabilité.",
    "incinerator.c4t":"Usines et Industries","incinerator.c4p":"Unités robustes adaptées aux installations sanitaires industrielles à fort trafic.",
    "incinerator.c5t":"Espaces Publics","incinerator.c5p":"Centres commerciaux, gares routières et gares ferroviaires — permettant une élimination sûre partout où des distributeurs automatiques sont installés.",
    "incinerator.c6t":"Élimination Écologique","incinerator.c6p":"Réduit la charge des décharges et la contamination environnementale due aux déchets sanitaires mal éliminés.",
...
/* ===================== SANITARY PAD MACHINES NAV DROPDOWN ===================== */
function toggleMachinesMenu(e) {
    if (e) e.preventDefault();
    const dropdown = document.getElementById('machinesDropdown');
    const menu = document.getElementById('machinesMenu');
    const willOpen = !menu.classList.contains('open');

    if (willOpen) {
        /* Position the menu using fixed coordinates computed from the toggle
           button's actual on-screen location, so it can never be clipped by
           any scrollable/overflow ancestor (e.g. the horizontally-scrolling
           mobile nav bar). */
...
/* Close machines dropdown when clicking outside of it */
document.addEventListener('click', function(e) {
    const dropdown = document.getElementById('machinesDropdown');

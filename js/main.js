...
async function getProducts() {
    ...
    /* If Supabase has products that no longer exist in code, or is missing new
       ones, reconcile the table in the background (non-blocking). */
    syncProductTable(finalProducts);
...
async function syncProductTable(products) {
    const rows = products.map(p => ({ id: p.id, name: p.name, meta: p.meta, tiers: p.tiers }));
    const { error } = await sb.from('products').upsert(rows);
    if (error) console.error('syncProductTable error:', error);
}
